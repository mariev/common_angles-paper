...
library(knitr)
options(width=32)
opts_chunk$set(fig.path='images/', cache.path='cache/', fig.align='center',  fig.width=5, fig.height=5, cache=TRUE)
@
%
%
\begin{figure}[hbtp]
...
<<parset-titanic-col, results='asis', echo=FALSE, out.width='.85\\linewidth', fig.width=8, fig.height=3.75>>=
library(ggparallel)
library(grid)
library(RColorBrewer)
titanic <- as.data.frame(Titanic)

cols <- c(brewer.pal(name="Blues", 6)[-c(1,2)], rev(brewer.pal(name="Oranges", 3)[-1]))
ggparallel(list("Survived", "Class"), weight="Freq", data=titanic, text.angle=0,col=NA, 
           order=c(1,0), method="parset", color="red") + coord_flip() + theme(legend.position="none") +    scale_fill_manual(values=rep("grey30",6)) + 
  scale_colour_manual(values=rep("grey70",6)) + 
  theme(plot.margin=unit(rep(0,4), units="cm"))


ggsave("images/aspect31-titanic.pdf", width=9, height=3)
ggsave("images/aspect33-titanic.pdf", width=3, height=3)
@
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=0.5\linewidth]{images/linewidth}
\end{center}
...
Hammock plots, introduced by  in \citet{schonlau:2003}, provide an alternative to parallel sets that is adjusted for the line width illusion. This is done by  adjusting the { \it horizontal} line width by  a factor of $\sin \theta$, as discussed in equation (\ref{adjust}). This adjustment makes the perceived {\it orthogonal} line width proportional to the number of observations it represents. 
 Figure \ref{hammock} shows an example of a hammock plot of the Titanic data. From top to bottom Class, Gender, Survival, and, again, Class are shown, capturing all two-way relationships of these variables. 
\begin{figure}
% <<hammocktitanic, fig.width=6, fig.height=8, out.width='\\linewidth', results='asis', echo=FALSE>>=
% cols <- c(brewer.pal(name="Blues", 6)[-c(1,2)],  rev(brewer.pal(name="Greens",3)[-1]), brewer.pal(name="Oranges", 3)[-1])
% ggparallel(names(titanic)[c(1,4,2,1)], order=c(0,1,1,0), method="hammock", ratio=.25, text.angle=0, titanic, weight="Freq") +
%   scale_fill_manual(values=cols, guide="none") +
%   scale_colour_manual(values=cols, guide="none") + coord_flip() + theme_bw() + theme(plot.margin=unit(rep(0,4), units="cm"))
% ggsave("revision/images/hammock-titanic.pdf", width=6, height=6)
% @
\centering
...
The structure of a hammock plot is the same as that of a parallel sets plot; 
 bars are divided according to class membership numbers, and  lines connect categories between neighboring variables. Unlike the parallel sets, it is now \emph{orthogonal} line widths that represent the number of individuals in each combination. Additionally, the lines start from the middle of the bin and connect to the middle of the other variable's bins. This convention is in part due to the fact that the sum of  horizontal widths ($w_h$) after adjustment is greater than the width of marginal bars.

The graph of figure~\ref{hammock} shows that barely any women were in the crew, while male crew members make up the second largest contingent overall. A few more men survived than women, but proportionally the situation is very different -- a much higher percentage of women survived than men. While more first class passengers survived than not, the  survival chances of second class passengers were evenly divided. More members of the third class and the crew perished than survived.

One problem that arises when dealing with hammock plots, is that the adjustment of lines depends on the knowledge of the angle $\theta$ under which each line is drawn. This depends on the aspect ratio of the plot. Any changes to the aspect ratio, such as a non-proportional resizing of a figure results in a change of the angle and results in a re-introduction of the linewidth illusion. 
...
\subsection{Reverse linewidth illusion}
A problem that arises in evaluating hammock plots is that if an observer focuses on horizontal line width  the plots suffer from a {\it reverse} of the {\it line width illusion}:  judging the number of survivors by class in figures \ref{hammock} and \ref{hammock_zoom} based on horizontal line width  results in an ordering of (largest to smallest) Crew, 3rd, 1st, and 2nd -- which is not the correct ordering: there were actually  more survivors in the 1st class than in the 3rd class. %Using horizontal width is inviting, since the lines are centered around the middle of a level, im
Unfortunately, the problem of the reverse linewidth illusion is exacerbated in hammock plots due to their construction:
the centering of lines around the middle of each level introduces a contextual coordinate system  that encourages comparisons of horizontal line widths, which are due to the adjustment no longer proportional to the number of records they represent.
The amount of the perceived distortion  can be quantified by rearranging equation~\ref{adjust} as:
...
\begin{figure}[htbp]
\centering
\includegraphics[width = .65\linewidth]{images/grid-hammock-zoom}
...
The idea behind the common angles plot is to construct a plot that is safe from both the line width illusion and its reverse, while keeping the basic structure of parallel sets plots and hammock plots.
The line width illusion  is triggered by differences in angles under which lines are drawn, so the basic idea is to draw all lines in a plot under the same angle. As this is not generally possible, a solution is to give up on the idea of straight lines between variable bars and think of ribbons instead, that are constructed from connecting line segments. We  can then ensure that all of these ribbons have at least one  line segment that is drawn under the same angle. Assuming vertical bars for the variables, connecting bands between  categories  are drawn as a combination of a vertical  segment, a  segment under a pre-specified angle $\theta$, followed by another vertical  segment as sketched out in figure~\ref{segments}
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
\centering
\includegraphics[width=0.8\linewidth]{images/segments} 
\caption{ \label{segments} Sketch of ribbons under three different angles (from left to right). All ribbons form a connection between points $A$ and $B$. }
  \end{figure}

\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
<<caplot, echo=FALSE, out.width='0.9\\linewidth', fig.width=6, fig.height=6>>=
cols <- c(brewer.pal(name="Blues", 6)[-c(1,2)],  rev(brewer.pal(name="Greens",3)[-1]), (brewer.pal(name="Oranges", 3)[-1]))
library(grid)
ggparallel(names(titanic)[c(1,4,2,1)], order=c(0,1,1,0), titanic, weight="Freq", text.angle=0) + 
 scale_fill_manual(values=cols, guide="none") +
 scale_colour_manual(values=cols, guide="none") + coord_flip() + theme_bw() + theme(plot.margin=unit(rep(0,4), units="cm"))
#ggsave("images/ca-titanic.pdf", width=6, height=6)
@
%   \centering
...
%In order to ensure that  widths of all bands are  comparable without any distortion, their slopes  are artificially made the same
...
Figure \ref{fig:ca-titanic} shows a common angle plot of the same data as the hammock plot. Now, tasks such as ordering levels according to the numbers they represent are not affected by either line width illusion and 13 out of 18 participants in the survey gave a correct assessment of the number of survivors by class (more details to follow in the next section).
...
Common angles, as well as all of the related methods of hammock plots and parallel sets are implemented in the  package \texttt{ggparallel} based on the \texttt{ggplot2} \citep{ggplot2} plotting framework in the software R 2.15.1 \citep{R}. The ggparallel package is freely available from CRAN (http://www. r-project.org/). The colors for the plots have been chosen using color schemes suggested by the ColorBrewer project \citep{colorbrewer}, as implemented in the R package RColorBrewer \citep{RColorBrewer}.
...
The study was conducted in form of a crossover design (see Table~\ref{tab:designs}): each participant was presented with two out of the three display types, where the first display showed the Titanic data, and the second display showed the gene data set. All participants were asked to answer the same set of questions (see Appendix~\ref{app2}) covering tasks I through III for each data set. 
% Each participant was presented with two of the three types of displays.
% For each display,  participants were asked to complete all of the three tasks.  % for each data set. 
% All participants were evaluated using the same set of questions with multiple choice options as detailed in Appendix \ref{app2}, regardless of display type or order. Participants were all shown the Titanic data first, then the gene data.  %one for all three tasks related to the same data set.
% This resulted in a crossover design consisting of  a total of six unique combinations of data and display as shown in Table \ref{tab:designs}. 
This design allows for comparisons of display types and tasks while it is possible to simultaneously adjust for individuals' different skill sets and learning effects.


At the start of the survey, participants were given a link to a brief tutorial regarding the different plot types. Not all of the participants followed this link.
The decision to not require participants to go through a thorough training beforehand was conscious. The main goal of our study was to assess performance of the plots  based on intuitive evaluation. We therefore refrained from any coaching on  how to evaluate plots in the training material and restricted ourselves to an explanation of the construction. XXX 
...
On average, participants needed 18 minutes to complete the survey. We did not find any significant differences between the amount of time needed between the first and the second block of questions ($F_{1, 95} = 0.0556$ for a $p$-value of 0.8142), nor were there significant differences in the length of time taken between the three designs ($F_{2, 94} = 0.1909$ for a $p$-value of 0.8265).

No personally identifiable information was collected, nor did we offer any compensation for participation in the survey.   
...
We are investigating four  aspects  of the experiment in this section: 
{\bf(i)}~participants' performance on each task according to the percentage of correct responses,
{\bf(ii)}~extent of variability due to subject-specific abilities,
{\bf(iii)}~space of answers for the more complex ordering task III, and
{\bf(iv)}~participants' preferences after completing tasks on two designs.
%first,  assessing general performance on the tasks according to percentage of correct responses, second, investigating the extent of variability due to subject-specific abilities, and finally exploring of the space of answers for the more complex ordering Task III.
...
The observed results match our expectations:
as we aimed for, task I does not show any significant differences between the designs and has overall the highest percentage of correctness reflecting its low difficulty level. Generally, the percentage of correct responses for a task  decreases with an increase of its complexity.

Parallel sets were affected the most by the line width illusion and show significantly worse performance for tasks II and III in both the Titanic and the genes data.

Hammock plots led to significantly worse performance than common angle plots in the two questions that were affected by the reverse line width illusion, while they show equal performance as common angle plots for the other questions. For task II in the genes data,  hammock plots have the overall best performance  across designs-- but this  does not  constitute a significant improvement over the performance of the common angle plot. Only for a sample size of over 150 participants would this difference be significant (for a power of  50\% to detect this difference).
The pathway-chromosome relationship is shown in a common angle plot in figure \ref{fig:caffeine}. The top bar identifies chromosome location, the bottom pathways. Ribbon widths  visualize the number of genes on a  chromosome that are active in a particular pathway. Task III for the genes data asked for a ordering of the chromosomes involved into the  pathway of steroid steroid.  

Figure~\ref{fig:model-summary} gives an overview of the performance of each design on all tasks. 

\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=\linewidth]{caffeine-ca} 
   \caption{\label{fig:caffeine}Common angle plot of the genes data. The top bar shows chromosome, the bottom the pathway. Width of lines corresponds to the number of genes pathways and chromosomes have in common. Both drug metabolism (hsa00982) and caffeine metabolism (hsa00232) make use of genes on chromosome 2. Task III of the survey asked for the order of the chromosomes involved into the steroid biosynthesis pathway from fewest number of genes to highest.}
\end{figure}
...
This results  in a  regular graph of degree six, where each  node is connected to six other nodes. Between any two nodes, the Cayley distance on the graph is equivalent to the length of the shortest connecting path between the two nodes.
...
the answers to different designs occupy quite different regions, while answers based  on the same design are quite close and  usually separated by only one edge. 
...
the area of a connecting line in parallel sets has a constant  overall area  independent of the angle under which it is drawn. Both hammock plots and common angle plots break this property. It does not seem, however, that the audience picks up on area as the main property of the displays. 
%This might be also influenced by the relative high difficulty of the task of comparing areas \citep{heer:2010, kong:2010}, in particular, if they suffer from  (1) extreme aspect ratios of the rectangular shape or (2) are drawn under different orientations, both of which apply to parallel sets. 
...
A drawback of this approach is that the sum of the bandwidths $w_h$ will no longer match the widths of the marginal bars, which might create an additional processing burden for the audience in mapping the relationship between 
band width and the width of marginal bars. 
...
% cols <- c(brewer.pal(name="Blues", 6)[-c(1,2)],  rev(brewer.pal(name="Greens",3)[-1]), (brewer.pal(name="Oranges", 3)[-1]))
% library(grid)
% ggparallel(names(titanic)[c(1,4,2,1)], method="adj.angle", order=c(0,1,1,0), ratio=0.04, titanic, weight="Freq", text.angle=0) + 
%     scale_fill_manual(values=cols, guide="none") +
%     scale_colour_manual(values=cols, guide="none") + coord_flip() + theme_bw() + theme(plot.margin=unit(rep(0,4), units="cm"))
\centering
...
%the presence of that illusion, rather it obfuscates the message of the display. 
In particular, a display with visual cues in conflict
with (the)textual) interactive feedback introduces a higher cognitive load by asking the audience to make a decision between one of the sources of information (and disregard the other one as incorrect).
...
indicate
that common angle plots effectively communicate underlying data without being affected by perceptual distortions due to the line width illusion or its reverse.
...
horizontal measure, leading to a reverse line width illusion. Common angles avoids the perceptual distortion 
...
\section{Survey Questions}\label{app2}
The tutorial shown to participants at the start of the survey can be found at \url{http://mariev.net/tutorial.html}.
