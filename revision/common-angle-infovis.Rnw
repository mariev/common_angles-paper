...
Hammock plots, introduced by  in \citet{schonlau:2003}, provide an alternative to parallel sets that is adjusted for the line width illusion. This is done by  adjusting the { \it horizontal} line width by  a factor of $\sin \theta$, as discussed in equation (\ref{adjust}). This adjustment makes the perceived {\it orthogonal} line width to be proportional to the number of observations it represents. 
